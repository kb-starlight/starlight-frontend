<template>
    ...
    <table class="table" >
        ...
        <tr>
            ...
            <td class="table1"> &nbsp; <input type="text" v-model="id" :disabled="id_check" class="textbox"> <button @click="ck_id">중복확인</button> <span class="notice1" v-if="checkid">4~12자의 영문
                    대소문자와 숫자로 입력</span>  <span class="notice" v-if="!checkid">사용가능한 아이디입니다.</span></td>
        </tr>
        <tr>
            <td class="green"> 비밀번호 </td>
            <td class="table1"> &nbsp; <input type="password" v-model="pw" class="textbox"> <span class="notice1" v-if="checkpw" >4~12자의 영문 대소문자와 숫자로 입력</span>
                <span class="notice" v-if="!checkpw">사용가능한 비밀번호입니다.</span>
            </td>
        </tr>
        <tr>
            <td class="green">  비밀번호 확인 </td>
            <td class="table1"> &nbsp; <input type="password" v-model="ckpw" class="textbox"><span class="notice1" v-if="checkpassword"> 비밀번호가 일치하지 않습니다. </span> 
                <span class="notice" v-if="!checkpassword"> 비밀번호가 일치합니다.</span></td>
        </tr>
        <tr>
            <td class="green"> 메일 주소</td> 
            <td class="table1"> &nbsp; <input type="text" v-model="email" class="textbox"><span class="notice" > 예 ) id@domain.com</span> 
                <span class="notice1" v-if="!ckEmail"> 이메일이 형식에 맞지 않습니다.</span> </td>
        </tr>
        
      
        <tr>
            <td class="green"> 이름 </td>
            <td class="table1"> &nbsp; <input type="text" v-model="name" class="textbox"> <span class="notice" > 이름은 한글만 입력 가능합니다.</span> <span class="notice1" v-if="!ckName"> 이름이 형식에 맞지 않습니다.</span></td>
        </tr>
        <tr>
            <td class="green"> 생년월일 </td>
            <td class="table1"> &nbsp; <input type="text"  v-model="idNum" class="textbox"><span class="notice">  ex) 20241231 </span>
                 <span class="notice1" v-if="ckIdNum"> / 8자리 숫자로 입력하세요</span></td>
        </tr>

        
        <tr>
            <td class="green"> 휴대전화번호 </td> 
            <td class="table1"> &nbsp; <input type="text" v-model="phone" class="textbox"><span class="notice">  예) 01012345678</span> <span class="notice1" v-if="!ckPhone"> / 숫자로만 입력하세요.</span></td>
        </tr>
        <tr>
            <td colspan="2">  </td>
        </tr>
...
export default {
    ...
    phone(){
        ...
        if(pattern.test(this.phone)&&(this.phone.length==11||this.phone.length==10)){
            this.ckPhone = true;
        }else{
            this.ckPhone = false;
        }
    ...
    methods:{
        ...
        ck_email(){
            ...
            if(pattern.test(this.email)){  //형태가 맞으면 true반환하는 메소드임.
                this.ckEmail = true;
            }else{
                this.ckEmail = false;
            }
